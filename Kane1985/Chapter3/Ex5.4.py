...
def eval_vec(v, N, s=None):
    vs = v.subs(s) if s is not None else v
...
# For each part A, B, C, D, define an origin for that part such that the centers
# of mass of each part of the component have positive N.x, N.y, and N.z values.
## FIND CENTER OF MASS OF A
vol_A_1 = pi * a**2 * b / 4
vol_A_2 = pi * a**2 * a / 4 / 2
vol_A = vol_A_1 + vol_A_2
pA_O = Point('A_O')
pAs_1 = pA_O.locatenew('A*_1', (b/2 * N.z +
                                centroid_sector.subs({R:a, theta:pi/4}) * sin(pi/4) *
                                (N.x + N.y)))
R_theta_val = {theta:pi/4*(1 - z/a), R:a}
pAs_2 = pA_O.locatenew('A*_2', (b * N.z +
                                N.z * integrate((theta*R**2*(z)).subs(R_theta_val),
                                                (z, 0, a)) / vol_A_2 +
                                N.x * integrate((theta*R**2*(
                                            centroid_sector*cos(theta))).subs(R_theta_val),
                                                (z, 0, a)) / vol_A_2 +
...
print('A* = {0}'.format(eval_vec(pAs.pos_from(pA_O), N, ab_val)))
...
pB_O = Point('B_O')
pBs_1 = pB_O.locatenew('B*_1', (a * (N.x + N.z) + a/2 * N.y +
                                (R*sin(theta)/theta * sin(pi/4)).subs({R:a, theta:pi/4}) *
                                 (-N.x + N.z)))
...
print('B* = {0}'.format(eval_vec(pBs.pos_from(pB_O), N, ab_val)))
...
pCs_3 = pC_O.locatenew('C*_3', (b*N.z +
                                a/2*(N.x + N.y + N.z)))
pCs_4 = pC_O.locatenew('C*_4', ((a+b)*N.z +
                                a/2*N.y + (N.x - N.z) * (centroid_sector.subs(
                                        {R:a, theta:pi/4}) * sin(pi/4))))
...
print('C* = {0}'.format(eval_vec(pCs.pos_from(pC_O), N, ab_val)))
...
print('D* = {0}'.format(eval_vec(pDs.pos_from(pD_O), N, ab_val)))
...
print('\nm* = {0}'.format(eval_vec(pms.pos_from(pO), N, ab_val)))
