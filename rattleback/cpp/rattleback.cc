                        ...
                        void *params)
{
  ...
  z[17] = -x[2 + 5];
  z[18] = -x[0 + 5];
  // Hand added dxdt[1] and dxdt[2]
  double dxdt[3] = {0.0,
                    x[5]*z[2] + x[7]*z[1],
                    x[1 + 5] + (x[5]*z[1] - x[7]*z[2])*z[3]};
  // End hand added stuff
  z[19] = -dxdt[1];
  z[20] = 2*x[2 + 5];
  z[21] = g*m;
  z[22] = 2*x[0 + 5];
...
void rattleback_outputs(simdata *sd, rattleback_params *p)
{
  double dxdt[8];
  double *x = sd->x;
  const double a = p->a,
  ...
  // Compute dxdt
  rattleback_ode(sd->t, sd->x, dxdt, static_cast<void *>(p));
