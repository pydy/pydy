...
\begin{equation*}
    \dot{s}_X sin(\theta) - \dot{s}_Y cos(\theta) + \frac{1}{2}l\omega = 0
\end{equation*}
...
\end{equation*}
Plotting the calculated constraint force $F_C$ with the static friction force
$F_{fr}$ in \autoref{fig:axleforce}, we see that no slip of the rear
wheels is not a good assumption.

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{figures/axle_force.eps}
    \caption{Constraint Force on Cart Axle}
    \label{fig:axleforce}
\end{figure}

\vspace{.5cm}

\begin{lstlisting}[caption={simulate\_cart.m}, label={lst:simcart}]
...
\begin{lstlisting}[caption={cart\_equationsofmotion.m}, label={lst:carteom}]
